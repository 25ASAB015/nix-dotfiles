...
# Incluye plugins para Wayland/Hyprland y captura de audio
{
  pkgs,
  config,
  lib,
...
in
{
  options.modules.media.obs = {
    enable = lib.mkEnableOption "OBS Studio with Wayland/Hyprland plugins";
  };
  ...
  config = lib.mkIf cfg.enable {
    home.packages = with pkgs; [
      ...
      v4l-utils
      # OBS Studio wrapped with plugins
      (wrapOBS {
        plugins = [
