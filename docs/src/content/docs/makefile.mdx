...
#### `make watch-rebuild`
Monitorea el proceso de rebuild en modo dry-run, actualizándose cada segundo.
...
```
════════════════════════════════════════════════════
          🔄 Watching Rebuild Process               
════════════════════════════════════════════════════

Refreshing every 1 second (dry-run mode)
Press Ctrl+C to exit

[Salida de nixos-rebuild dry-run actualizándose cada segundo]
```

**¿Para qué sirve?**
- **Monitoreo continuo:** Observa el estado del rebuild sin ejecutarlo realmente
- **Verificación previa:** Revisa qué cambios se aplicarían antes de hacer `make switch`
- **Debugging:** Identifica problemas de configuración mientras se resuelven
- **Aprendizaje:** Entiende qué hace NixOS durante un rebuild

**Características:**
- Usa `nixos-rebuild dry-run` para simular el rebuild sin aplicarlo
- Se actualiza cada segundo usando `watch`
- Muestra las últimas 20 líneas de la salida
- Formato consistente con otros comandos de monitoreo
- Modo seguro: no modifica el sistema, solo muestra qué pasaría

**Cuándo usarlo:**
- Antes de hacer un `make switch` para verificar cambios
- Mientras editas archivos de configuración para ver el impacto en tiempo real
- Para entender qué paquetes se construirían o descargarían
- Cuando quieres monitorear el progreso de un rebuild sin ejecutarlo

**Nota:** Este comando usa `dry-run`, por lo que no modifica el sistema. Para aplicar los cambios, usa `make switch` en otra terminal.
