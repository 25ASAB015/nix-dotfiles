...
```bash
make switch-safe # Todo-en-uno: valida + aplica
```

**Flujo 3: Testing antes de permanente**
```bash
make test        # Probar temporalmente
make switch-safe # Si funciona, hacer permanente
```
...
- **`make switch-safe`** → Para cambios importantes o cuando no estás seguro
...
- **Cuando `/nix/store` está grande**: Verifica con `make status` el tamaño del store
...
**Recomendación:** Si es tu primera vez o buscas máxima seguridad, usa `make clean DAYS=90`.
...
|---------|-----------|----------------|-----------|
| `make clean` | `DAYS=90` | 90 días | ⭐⭐⭐⭐⭐ Máxima |
| `make clean` | (Default) | 30 días | ⭐⭐⭐⭐ Alta |
| `make clean` | `DAYS=7` | 7 días | ⭐⭐ Media |
| `make deep-clean`| N/A | 0 días | ⭐ Mínima |
| `make optimize` | N/A | N/A | ⭐⭐⭐⭐⭐ Seguro |
...
2. **Verifica antes y después**: Usa `make status` para ver el impacto
...
7. **Monitorea el espacio**: Revisa periódicamente con `make status` o `df -h /nix/store`
...
- Más rápido que `make switch` estándar cuando el sistema ya tiene las dependencias en el store
...
- `make switch-safe` - Valida antes de aplicar, más seguro aún
- `make switch-fast` - Sin verificaciones, más rápido y omite verificaciones internas de nixos-rebuild
...
make update              # Actualizar entradas
make flake-diff          # Ver qué paquetes cambiarán
make validate            # Verificar compatibilidad
make switch              # Aplicar
make clean               # Liberar espacio
make optimize            # Deduplicar store
```
...
**Nota:** La información de System Info ahora se muestra de forma organizada en `make status`.
...
make switch-safe    # Valida y aplica
```
