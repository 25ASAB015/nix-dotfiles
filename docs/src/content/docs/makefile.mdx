...
#### `make rebuild`
Alias directo de `make switch` que reconstruye y aplica la configuración. Es funcionalmente idéntico a `make switch`.
...
**Qué hace:**
- Ejecuta exactamente lo mismo que `make switch`
- Compila la configuración completa
- Aplica los cambios permanentemente
- Crea una nueva generación del sistema

**Output ejemplo:**
```
==================== Rebuild ====================
🔄 Reconstruyendo configuración de NixOS...
...
**Cuándo usarlo:**
- Cuando prefieres el nombre "rebuild" por claridad semántica
- Scripts o alias que esperan el comando `rebuild`
- Cuando quieres ser explícito sobre que estás reconstruyendo el sistema
- Compatibilidad con convenciones de otros proyectos

**Diferencias con `make switch`:**
- **Funcionalmente:** Ninguna diferencia, hacen exactamente lo mismo
- **Semánticamente:** `rebuild` enfatiza "reconstrucción", `switch` enfatiza "cambio"
- **Uso:** `switch` es más común, `rebuild` puede ser más claro para algunos usuarios

**Nota:** Este comando es un alias puro. Si necesitas funcionalidad adicional (como validación), usa `make safe-switch` en su lugar.
...
#### `make restore` ⚠️ NO IMPLEMENTADO
Este comando **no está implementado** en el Makefile actual. Para restaurar versiones anteriores, usa las alternativas documentadas abajo.

**Alternativas disponibles:**

**Opción 1: Rollback a generación anterior del sistema**
```bash
make rollback
```
- Revierte la configuración del sistema a la generación anterior
- Cambios disponibles inmediatamente
- No afecta archivos de configuración editados manualmente

**Opción 2: Restaurar archivos específicos con Git**
```bash
# Ver historial de cambios
git log --oneline

# Restaurar archivo específico de un commit anterior
git checkout <commit-hash> -- path/to/file.nix

# Aplicar cambios
make switch
```

**Opción 3: Restaurar múltiples archivos**
```bash
# Restaurar todos los archivos de un commit anterior
...
**Opción 4: Seleccionar generación en GRUB**
```bash
# Listar generaciones disponibles
make list-generations

# Reiniciar y en GRUB seleccionar generación anterior
# (más atrás que la inmediatamente anterior)
```

**¿Por qué no está implementado?**
- `make rollback` cubre el caso más común (volver a generación anterior)
- Git es más flexible para restaurar archivos específicos
- Evita confusión sobre qué restaura exactamente
- Mantiene el Makefile simple y enfocado

**Recomendación:** Usa `make rollback` para revertir cambios del sistema, y Git para restaurar archivos de configuración específicos.
...
- **Seguimiento durante rebuilds:** Observar logs mientras se ejecuta `make switch` o `make rebuild` en otra terminal
...
**Cuándo usarlo:**
- Durante un `make switch` o `make rebuild` para ver qué está pasando
