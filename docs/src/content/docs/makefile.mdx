...
description: Documentación del Makefile - ahora modularizada
---

# Makefile: Documentación Modular

La documentación del Makefile ha sido modularizada para facilitar la navegación y el mantenimiento.

**👉 [Ver documentación completa del Makefile](/nix-dotfiles/makefile/)**

## Módulos Disponibles

- [Overview](/nix-dotfiles/makefile/) - Introducción y navegación
- [Ayuda y Documentación](/nix-dotfiles/makefile/01-docs/) - help, help-examples, docs-*
- [Gestión del Sistema](/nix-dotfiles/makefile/02-system/) - switch, test, build, validate
- [Limpieza y Optimización](/nix-dotfiles/makefile/03-cleanup/) - clean, optimize
...
- [Git y Respaldo](/nix-dotfiles/makefile/06-git/) - git-*, save
- [Diagnóstico y Logs](/nix-dotfiles/makefile/07-logs/) - health, test-network, logs-*
- [Análisis y Desarrollo](/nix-dotfiles/makefile/08-dev/) - search, repl, vm
...
**Ventajas:**
- ✅ Único comando que activa temporalmente (diferente de `make build`)
...
**Nota importante:** A diferencia de `make build` que solo compila sin activar, `make test` compila **Y activa** temporalmente. Esto te permite probar la configuración en un entorno real pero sin consecuencias permanentes.
...
- Validación: comprobar cambios antes de hacer `make switch`
...
- `make build`: Solo compila, no activa nada, incluye medición de tiempo
...
**Relación con `make build`:**
- `make build` crea symlinks `result*` que apuntan a los builds
- `make clean-result` elimina estos symlinks después de usarlos
...
- Después de `make update` para ver qué se actualizaría
...
|---------|---------------------|-------------------|----------------|
| `make switch` | ✅ Sí (inmediato) | ❌ No | Ahora mismo |
| `make boot` | ❌ No | ✅ Sí | Próximo arranque |
| `make test` | ✅ Sí (temporal) | ❌ No | Solo esta sesión |
...
**Nota importante:** Después de `make boot`, debes reiniciar el sistema para que los cambios se apliquen. Si cambias de opinión antes de reiniciar, puedes usar `make switch` para aplicar cambios inmediatamente o `make rollback` si necesitas revertir.
...
- **Confianza:** Te asegura que la configuración es válida antes de hacer `make switch`
...
**Cuándo usarlo:**
- Antes de cada `make switch` para asegurar que la configuración es válida
...
- Después de `make update` (cuando hay cambios en `flake.lock`)
...
- ❌ Requiere que ejecutes `validate` manualmente
...
**Recomendación de uso:**
- **`make switch`** → Para cambios pequeños y frecuentes (desarrollo iterativo)
- **`make safe-switch`** → Para cambios importantes o cuando no estás seguro

**Ver también:** [Flujo de Trabajo Recomendado](#flujo-recomendado) para más detalles sobre cuándo usar cada comando.

#### `make health`
Chequeo rápido de salud del sistema que verifica aspectos críticos sin operaciones costosas.
...
- `make list-hosts` - Muestra información detallada de cada host (usa `make show` internamente)
...
- **Cuando `/nix/store` está grande**: Verifica con `make info` el tamaño del store
...
#### `make clean-week`
Limpieza más agresiva - mantiene solo últimos 7 días de historial.
...
**Qué elimina:**
- Generaciones mayores a 7 días
- Paquetes no referenciados
- Build artifacts antiguos

**Espacio liberado típico:** 10-40 GB (más que `make clean`)

**⚠️ Cuidado importante:**
- Solo podrás hacer rollback a generaciones de la última semana
- Si necesitas volver a una configuración de hace más de 7 días, no podrás
- Útil cuando necesitas espacio rápidamente pero mantienes algo de seguridad

**Cuándo usarlo:**
- **Poco espacio en disco**: Necesitas liberar espacio rápidamente
- **Muchos rebuilds recientes**: Has estado experimentando y tienes muchas generaciones nuevas
- **Testing intensivo**: Has hecho muchas pruebas y las generaciones antiguas no son críticas
- **Sistema de desarrollo**: Donde la estabilidad a largo plazo no es crítica
...
Limpieza muy segura - mantiene últimos 90 días de historial.
...
**Qué elimina:**
- Solo generaciones y paquetes mayores a 90 días
- Mantiene 3 meses completos de historial

**Espacio liberado típico:** 2-10 GB (menos que `make clean` pero más seguro)

**Ventajas:**
- Máxima seguridad para rollback
- Permite volver a configuraciones de hasta 3 meses atrás
- Ideal para sistemas donde la estabilidad es crítica

**Cuándo usarlo:**
- **Primera vez que limpias**: Si nunca has limpiado antes, empieza con esto
- **Sistema de producción crítico**: Donde necesitas máxima capacidad de rollback
- **Quieres máxima seguridad**: Prefieres mantener más historial que liberar espacio
- **Después de cambios importantes**: Cuando quieres mantener un historial largo por seguridad

**Recomendación:** Si no estás seguro de qué comando usar, empieza con `make clean-conservative`.
...
- **Complemento de limpieza**: Después de `make clean` para maximizar espacio
...
| Comando | Días mantenidos | Espacio liberado | Seguridad | Uso recomendado |
|---------|----------------|------------------|-----------|----------------|
| `make clean-conservative` | 90 días | 2-10 GB | ⭐⭐⭐⭐⭐ Máxima | Producción, primera vez |
| `make clean` | 30 días | 5-20 GB | ⭐⭐⭐⭐ Alta | Uso general, rutina mensual |
| `make clean-week` | 7 días | 10-40 GB | ⭐⭐ Media | Desarrollo, poco espacio |
| `make deep-clean` | 0 días (TODO) | 20-100+ GB | ⭐ Mínima | Emergencias únicamente |
| `make optimize` | N/A (no elimina) | 1-5 GB | ⭐⭐⭐⭐⭐ Seguro | Después de limpieza, rutina |
...
# make deep-clean  # Solo si estás 100% seguro
```
...
2. **Verifica antes y después**: Usa `make info` para ver el impacto
...
7. **Monitorea el espacio**: Revisa periódicamente con `make info` o `df -h /nix/store`
...
- ⚠️ Si necesitas volver más atrás, usa `make list-generations` y selecciona en GRUB
...
- `make diff-generations` - Comparar generación actual con anterior
- `make switch` - Aplicar cambios (puedes revertir con rollback)
- Git - Para restaurar archivos de configuración específicos

**Nota importante:** `make rollback` solo revierte la configuración del sistema. Si editaste archivos de configuración manualmente y quieres restaurarlos, usa Git:
```bash
...
**Cuándo usarlo:**
- Después de hacer `make switch` para ver qué cambió
...
**Relación con otros comandos:**
- `make diff-generations` - Compara automáticamente la generación actual con la anterior
- `make diff-gen GEN1=N GEN2=M` - Compara dos generaciones específicas que tú elijas
...
- `save` - Workflow completo: add + commit + push + rebuild
...
#### `make quick`
Rebuild rápido omitiendo verificaciones de seguridad para acelerar el proceso.
...
- Omite verificaciones y validaciones previas
- Aplica cambios directamente sin checks de seguridad
- Más rápido que `make switch` estándar
...
- **Ya validaste antes**: Después de `make validate` o `make test` exitosos
...
**Comparación con otros comandos:**
- `make switch` - Incluye verificaciones básicas, más seguro
- `make safe-switch` - Valida antes de aplicar, más seguro aún
- `make quick` - Sin verificaciones, más rápido pero menos seguro
...
- **Errores críticos**: Problemas que `make debug` no pudo resolver
...
- Cambios simples (usa `make switch` o `make quick`)
- Cuando `make debug` es suficiente

**Comparación con otros comandos:**
- `make debug` - Verbosidad moderada, útil para la mayoría de casos
...
- `make clean` - Mantiene 30 días, seguro para uso regular
- `make clean-week` - Mantiene 7 días, más agresivo pero seguro
- `make clean-conservative` - Mantiene 90 días, máxima seguridad
...
**Cuándo usarlo:**
- Durante un `make switch` para ver qué está pasando
...
**Nota:** Este comando se ejecuta automáticamente en `make switch` para prevenir problemas de permisos.
...
**Nota:** Este comando se ejecuta automáticamente en `make fix-permissions` y `make switch`.
...
make rollback            # Volver a generación anterior
...
### Git y Respaldo (7)
`git-add`, `git-commit`, `git-push`, `git-status`, `git-diff`, `save`

### Diagnóstico y Logs (8)
