...
#### `make gc`
Alias de `make clean` (garbage collection estándar). Mismo comportamiento que `make clean`.

```bash
make gc
```

**Nota:** `gc` es un alias común en sistemas Unix para "garbage collection". Este comando simplemente llama a `make clean` internamente.
...
Elimina generaciones específicas del sistema, mantiene últimos 14 días.
...
**Qué hace:**
- Elimina generaciones del sistema mayores a 14 días
- Mantiene las generaciones recientes para rollback
- Ejecuta garbage collection después de eliminar generaciones

**Balance perfecto:**
- Más espacio que `clean-conservative` (90 días)
- Más seguridad que `clean-week` (7 días)
- 14 días es suficiente para la mayoría de casos de uso

**Cuándo usarlo:**
- **Mantenimiento regular**: Cuando quieres un balance entre espacio y seguridad
- **Después de estabilizar cambios**: Cuando has probado cambios y quieres limpiar generaciones de prueba antiguas
- **Sistema personal**: Donde 14 días de historial es suficiente

**Ver generaciones antes de limpiar:**
```bash
# Ver qué generaciones tienes
...
| `make clean-generations` | 14 días | Variable | ⭐⭐⭐ Media-Alta | Balance espacio/seguridad |
...
# Limpieza de generaciones (14 días) - balance
make clean-generations

# Garbage collection (alias de clean)
make gc
