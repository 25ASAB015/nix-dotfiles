...
### `make upd-ai`
Actualiza solo las herramientas de IA: **OpenCode**, **Cursor** y **Antigravity**.
...
- **nixpkgs-unstable** → Cursor y Antigravity (editores, vía overlay en `modules/system/editor-overlays.nix`)

**Flujo recomendado:**
```bash
make upd-ai      # Actualiza opencode y nixpkgs-unstable
make upd-diff    # (Opcional) Ver qué cambió en flake.lock
make sys-apply   # Aplicar cambios al sistema
```
